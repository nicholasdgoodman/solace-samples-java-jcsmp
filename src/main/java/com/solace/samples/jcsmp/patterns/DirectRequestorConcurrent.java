 ...
 */
public class DirectRequestorConcurrent {

    private static final String SAMPLE_NAME = DirectRequestorConcurrent.class.getSimpleName();
    ...
    public static void main(String... args) throws JCSMPException, IOException {
        ...
        final Requestor requestor = session.createRequestor();  // create the useful Requestor object, Direct only
        
        // topic should look like 'solace/samples/jcsmp/direct/request'
        final String topicString = TOPIC_PREFIX + API.toLowerCase() + "/direct/request";
        final Topic topic = JCSMPFactory.onlyInstance().createTopic(topicString);
        ...
        while (System.in.available() == 0 && !isShutdown) {
            try {
                ...
                executor.submit(() -> {
                    try {
                        int messageNum = msgSentCounter;
                        TextMessage requestMsg = JCSMPFactory.onlyInstance().createMessage(TextMessage.class);
                        requestMsg.setText(String.format("Hello, this is reqeust #%d", messageNum));
                        System.out.printf(">> About to send request message #%d to topic '%s'...%n", messageNum, topicString);
                        ...
                        Thread.sleep(REQUEST_PERIOD_MS);  // approx. one request per second per thread
